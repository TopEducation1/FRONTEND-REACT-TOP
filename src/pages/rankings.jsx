            ...
            {ranking.entradas?.length > 0 ? (
              <ul>
                {ranking.entradas.map((entry) => {
                  const imgSrc = entry.universidad?.univ_ico || entry.empresa?.empr_ico;
                  ...
                  const isUniversidad = !!entry.universidad;

                  return (
                    <li
                      className="p-4 flex flex-wrap items-center justify-center lg:justify-start bg-[#1B1B1B] rounded-xl mb-4 relative"
                      key={entry.id}
                    >
                      <div className="w-full lg:w-[10%]">
                        {hasImage ? (
                          <img
                            src={imgSrc}
                            className="w-[120px] h-[120px] mx-auto mb-5 lg:mb-0 lg:w-[80px] lg:h-[80px] lg:ml-[35px] object-contain rounded-xl relative z-10 cursor-pointer"
                            alt={entry.universidad?.nombre || entry.empresa?.nombre || ""}
                            onClick={() =>
                              handleItemMenuClick({
                                [ranking.tipo
                                  ? ranking.tipo.charAt(0).toUpperCase() + ranking.tipo.slice(1)
                                  : ""]:
                          ...
                          <div
                            className="w-[120px] h-[120px] mx-auto mb-5 lg:mb-0 lg:w-[80px] lg:h-[80px] lg:ml-[35px] rounded-full bg-neutral-200 flex items-center justify-center text-3xl font-semibold text-neutral-600 relative z-10 cursor-pointer"
                            onClick={() =>
                              handleItemMenuClick({
                                [ranking.tipo
                                  ? ranking.tipo.charAt(0).toUpperCase() + ranking.tipo.slice(1)
                        ...
                        <strong className="bg-[#F6F4EF] text-black flex items-center rounded-[25px] lg:rounded-[25px_0px_0px_25px] w-auto !py-[3px] px-3 sm:pr-7 !lg:w-[60px] !lg:h-[60px] absolute top-[125px] lg:top-[30%] left-[50%] transform -translate-x-1/2 lg:translate-x-0 lg:left-2 z-11 lg:z-0 text-[0.9rem] lg:text-[1.5rem]">
                          <span className="block text-[0.9rem] mr-2 lg:hidden">Rank</span>
                          {entry.posicion}
                        </strong>
                      </div>

                      <div
                        className="flex flex-wrap items-center justify-center lg:justify-start w-[100%] lg:w-[35%]"
                        onClick={() =>
                          handleItemMenuClick({
                            [ranking.tipo
                              ? ranking.tipo.charAt(0).toUpperCase() + ranking.tipo.slice(1)
                      ...
                      >
                        <div className="w-full flex flex-wrap justify-center lg:justify-start lg:block">
                          <h2 className="text-[1.5rem] lg:text-[1.8rem] w-full font-semibold text-white text-center lg:text-left leading-[1.1em] mb-2">
                            {entry.universidad?.nombre || entry.empresa?.nombre}
                          </h2>
                      ...
                      <div className="w-full mt-10 lg:mt-0 lg:w-[55%] flex !items-center relative">
                        ...
                        >
                          {entry.temas_certificaciones?.map((tema) => (
                            <SwiperSlide
                              ...
                              onClick={() =>
                                handleItemMenuClick({
                                  [tema.tema_certificacion__tem_type]:
                                    tema.tema_certificacion__nombre,
                                  [ranking.tipo
                                    ? ranking.tipo.charAt(0).toUpperCase() + ranking.tipo.slice(1)
                                    : ""]:
                                    entry.universidad?.nombre || entry.empresa?.nombre,
                            ...
                            >
                              <div className="flex items-center px-2 gap-2">
                                {tema?.tema_certificacion__tem_img &&
                                  tema.tema_certificacion__tem_img !== "None" &&
                                  tema.tema_certificacion__tem_img.trim() !== "" && (
                                    <img
                                ...
                                <div className="w-[80%]">
                                  <span className="text-[#F6F4EF] text-[0.9rem] block w-full !leading-[1.1em]">
                                    {tema.tema_certificacion__nombre}
                                  </span>
                        ...
                        <div className="flex justify-between mt-2 absolute gap-1 top-[-30px] lg:top-[-35px] right-[50%] lg:right-1 transform translate-x-1/2 lg:translate-x-0 ">
                          <button
                            className={`boton-prev-${entry.id} bg-[#F6F4EF] hover:bg-[#F6F4EF]/70 text-[0.8rem] px-3 py-1 rounded-xl`}
                          >
                            <FaAngleLeft />
                          </button>
                          <button
                            className={`boton-next-${entry.id} bg-[#F6F4EF] hover:bg-[#F6F4EF]/70 text-[0.8rem] px-3 py-1 rounded-xl`}
                          >
                            <FaAngleRight />
                          </button>
                        </div>
